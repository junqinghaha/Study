...
import com.android.tnt.hmdy.R.color;
 ...
 * 
 */
public class PrintOnlyOneFragment extends WebServiceFragment implements View.OnClickListener {
	...
	private NumberSelectedView numberChildSelectView = null;
	...
	public void printOpt() {
		if (isDataOk()) {
			if (isAdd) {
				saveHistoryInfo();
				int childIndex = numberChildSelectView.getTextValue();
				goodsPrint(getNewEntity(), childIndex);
			} else {
				saveHistoryInfo();
				int childIndex = numberChildSelectView.getTextValue();
				goodsPrint(getEditEntity(), childIndex);
			}
		}
	}
	 ...
	 * @return
	 */
	private void goodsPrint(HuoDanEntity entity, int nIndex) {
		if (entity != null) {
			MApplication app = (MApplication) getActivity().getApplication();
			...
			if (!TextUtils.isEmpty(blDeviceAdress)) {
				if (PrintUtil.isPrinterOk(getActivity(), blDeviceAdress)) {
					PrintUtil.PrintOne(getActivity(), blDeviceAdress, entity, nIndex);
				} else {
	...
	private void initViews(View rootView) {
		...
		//numberChildSelectView = (NumberSelectedView) rootView.findViewById(R.id.et_03_2);
		if(entity != null){
			numberChildSelectView.init(0, 1, entity.getNum(), 1);
		}else{
			numberChildSelectView.init(0, 1, -1, 1);
		}
	...
	private boolean isNumberChildDataOk() {
		boolean bRt = false;
		int value = -1;
		String data = numberChildSelectView.getText().trim();// (R.id.et_03);//
		// TextUtils.isEmpty(;
		if (!TextUtils.isEmpty(data)) {
			try {
				value = Integer.parseInt(data);
	...
	private void toOpt(int optCode, Bundle data) {
		this.mFragmentlistener.onFragmentRequest(PrintOnlyOneFragment.this, optCode, data);
	}
