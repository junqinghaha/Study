...
import com.android.tnt.hmdy.adapter.MAutoCompleteArrayAdapter;
 ...
 * 
 */
public class AddFragment extends WebServiceFragment {
	...
	public void saveOpt() {
		if (isDataOk()) {
			if (isAdd) {
				...
				// 启动网络请求
				try {
					editWebservice.setParams(getEditEntity());
	...
	private void initViews(View rootView) {
		...
		// 始发站
		try {
			defValue = SysConfig.curUser.getStationName();
			if (entity != null) {
	...
	private boolean isOriginDataOk() {
		...
		try {
			if(TextUtils.isEmpty(value) && name.equalsIgnoreCase(SysConfig.curUser.getStationName())){
				value = SysConfig.curUser.getStationID();
			}
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	private void initAutoComplete(String field, AutoCompleteTextView auto) {
		...
		if (hisArrays != null) {
			MAutoCompleteArrayAdapter<String> adapter = new MAutoCompleteArrayAdapter<String>(getActivity(), android.R.layout.simple_dropdown_item_1line, hisArrays);
			// 只保留最近的50条的记录
			if (hisArrays.length > 100) {
				String[] newArrays = new String[100];
				System.arraycopy(hisArrays, 0, newArrays, 0, 100);
				adapter = new MAutoCompleteArrayAdapter<String>(getActivity(), android.R.layout.simple_dropdown_item_1line, newArrays);
			}
	...
	private void initAutoComplete(String[] arrDatas, String[] arrValues, String field, final AutoCompleteTextView auto) {
		if (arrDatas != null) {
			MAutoCompleteArrayAdapter<String> adapter = new MAutoCompleteArrayAdapter<String>(getActivity(), android.R.layout.simple_dropdown_item_1line, arrDatas);
			auto.setAdapter(adapter);
			// auto.setDropDownHeight(350);
			auto.setThreshold(1);
			auto.setOnFocusChangeListener(new OnFocusChangeListener() {
	...
	private void toOpt(int optCode, Bundle data) {
		this.mFragmentlistener.onFragmentRequest(AddFragment.this, optCode, data);
	}
	...
	protected void webserviceCallSucess(String methodName, String result) {
		...
		if (methodName.equalsIgnoreCase(addWebservice.getMethodName())) {
			...
			if ((Boolean) editWebservice.parseWebResult(result)) {
				save(entity);
				Bundle data = new Bundle();
				data.putString(Constants.KEY.DATA1, entity.getCode());
				// data.putString(Constants.KEY.DATA2, entity.getCode());
