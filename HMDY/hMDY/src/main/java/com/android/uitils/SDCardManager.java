...
public class SDCardManager {
	...
	public static String GetSDCardPath() {
		if (android.os.Build.VERSION.SDK_INT > 18) {// 4.4及以上

			// File[] cacheFile = context.getExternalCacheDirs();
			...
			// }
			// }
			// return files[1].getParentFile().getParent();
			...
			File[] files = new File(sdPath).listFiles(new FileFilter() {
				...
				public boolean accept(File pathname) {
					...
					if (pathname.isDirectory()
							&& pathname.getName().equals(folderName)) {
						return true;
					}
					return false;
				}
